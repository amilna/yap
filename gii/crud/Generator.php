...
class Generator extends \yii\gii\Generator
{
	...
	{
		$ms = get_class_methods ($this->modelClass);
		$tabs = [false];
		foreach ($ms as $m) {
			if ($m == 'loadDefaultValues') {
				break;	
			}
			elseif (substr($m,0,3) == 'get') {
				array_push($tabs,strtolower(substr($m,3)));	
			}
		}
		...
		//$db = Yii::$app->db;                
        //$tn = str_replace(["{{%","}}"],[$db->tablePrefix,""],$mod::tableName());
    ...
    public function generateSearchConditions()
    {
        ...
        if (!empty($likeConditions)) {
            $conditions[] = "\$params = self::queryString([".implode(",",$likeConditions)."".(count($lf) == 0?"":"/*".implode(",",$lf)."*/")."]);
		foreach (\$params as \$p)
		{
			\$query->andFilterWhere(\$p);
		}";
        ...
        if (!empty($timeConditions)) {
            $conditions[] = "\$params = self::queryTime([".implode(",",$timeConditions)."".(count($tf) == 0?"":"/*".implode(",",$tf)."*/")."]);
		foreach (\$params as \$p)
		{
			\$query->andFilterWhere(\$p);
		}";
